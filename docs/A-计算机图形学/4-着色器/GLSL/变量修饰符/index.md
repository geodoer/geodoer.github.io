...
在GLSL中，`varying`和`flat`都用于从顶点着色器传递数据到片元着色器，不同之处在于它们的插值方式不同。
...
-   `varying`变量会在图元中进行插值计算，生成一个平滑的过渡效果，而`flat`变量则不会进行插值计算，保持在整个图元中具有相同的属性值
-   `varying`变量可以是任意类型的（标量、向量或矩阵），而`flat`变量只能是标量或向量
